...
class Washers extends React.Component {
  ...
  getUsers() {
    get('http://localhost:9091/user/getCarWasher')
      .then(({ data }) => {
  ...
  render() {
    ...
    return (
      ...
      <Page title="Car Washer User List">
        <Helmet>
          <title>Car Washer User List</title>
        </Helmet>
        ...
        <Link to="/admin/addWasher"> Add New Car Washer</Link>
      </Page>
      </div>
      </div>
    );
